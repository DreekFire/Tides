...
local weaponDef = {
    { name = "CB1",
      velocity = 1649,
      checkACBs = {
        { axis = "Check1", minRange = 200, maxRange = 4000, turretName = "check1Turret", offset = Vector3(0, 1, 0), },
      },
      checkRequire = 1,
      validRequire = 1, },
    { name = "CB2",
      velocity = 1605,
      checkACBs = {
        { axis = "Check2", minRange = 200, maxRange = 4000, turretName = "check2Turret", offset = Vector3(0, 1, 0), },
      },
      checkRequire = 1,
      validRequire = 1, }
}
      ...
      for acbIdx, acb in ipairs(weaponDef[i].checkACBs) do
        ...
        if r.magnitude > acb.minRange and r.magnitude < acb.maxRange
          ...
          if I:GetCustomAxis(acb.axis) > 0 then
            success = success + 1
            I:Log(weaponDef[i].name .. " ACB Target confirmed.")
          else
            I:Log(weaponDef[i].name .. " ACB Target failed.")
          end
        end
      end
      ready = valid >= weaponDef[i].validRequire and success >= weaponDef[i].checkRequire * valid
    end
    if ready and wfp then
        ...
        if Combat.CheckConstraints(I, aim, weapon.wpnIdx, weapon.subIdx) then
          ...
          if fire then
            if Vector3.Angle(wInfo.CurrentDirection, aim) < AIM_TOL then
              if I:GetCustomAxis(stabilityAxisName) >= minStabilityToFire then
                I:Log("Fire!")
          ...
          --I:Log("Holding fire for mystery reason")
          end
        end
      end
    end
