...
function Update(I)
  ...
  if currentLine and CheckAndUpdateLine(I, currentLine, projectile, frameTime, EPSILON_P) then
    ...
    --I:Log("line duration: "..currentLine.dt)
    --LogVector(I, target.Position, "target position: ")
    --LogVector(I, projectile, "projectile position: ")
    --LogVector(I, I:GetConstructPosition(), "own position: ")
    --LogVector(I, relVel, "projectile velocity relative to target: ")
    --LogVector(I, relPos, "projectile position relative to target: ")
...
function CheckAndUpdateLine(I, line, projectile, frameTime, tolerance)
  local dt = I:GetTimeSinceSpawn() - line.tStart
  local expected = line.start + (line.ds * dt / line.dt)
...
function RunTrace(I, line, enemy, timeGuess)
  ...
  for i = 1, timeGuess and 1 or 2 do
    -- dx(t) = line.ed.x - absPos.x + t * (line.ds.x / line.dt - line.dv * Vector3.up - absVel.x)
    -- dz(t) = line.ed.z - absPos.z + t * (line.ds.z / line.dt - line.dv * Vector3.up - absVel.y)
    -- dy(t) = line.ed.y - absPos.y + t * (line.ds.y / line.dt - line.dv * Vector3.up - absVel.z) + 0.5 * I:GetGravityForAltitude(line.ed.y).y * t * t

    -- dx^2 = dxi^2 + 2t dxi vx + (vx t)^2
    -- dz^2 = dzi^2 + 2t dzi vz + (vz t)^2
    -- dy^2 = dyi^2 + 2t dyi vy + (vy t)^2 + (dyi + vy t) a t^2 + 0.25 * a^2 * t^4
    -- sqrdistance = dx^2 + dy^2 + dz^2

    -- d/dt sqrdistance = 
    -- 2 dxi vx + 2 vx^2 t +
    -- 2 dzi vz + 2 vz^2 t +
    -- 2 dyi vy + 2 vy^2 t + 2 dyi a t + 3 vy a t^2 + 0.25 a^2 t^3
