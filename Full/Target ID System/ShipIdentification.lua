-- WIP: ship identification based on stats measured through target prioritization cards.
-- todo: instead of measuring all priorities, use PCA to find minimal number of principal components
-- fewer components allows for fewer target prioritization cards and also shorter identification time
-- when AI wireless receivers are toggled the game takes about 0.5 seconds to recalculate connections
-- so measuring each component takes about 0.5s.
...
local mainframeChannel = 3
local mainframeIdx = 3
local sqrThreshold = 5
...
local enemyData -- defined at bottom because it's very long
local targets = {}
local receivers = {}
local currentStatIdx = 1
local switchingTime = 1
local lastSwitchTime

function Init(I)
  lastSwitchTime = I:GetGameTime()
  for i, name in ipairs(dataNames) do
    receivers[name] = BlockUtil.getBlocksByName(I, name, 23, 1)[1]
  end
end
...
function Update(I)
  ...
  if time - lastSwitchTime > switchingTime then
    GatherData(I, dataNames[currentStatIdx])
    currentStatIdx = currentStatIdx + 1
    if currentStatIdx > #dataNames then
      currentStatIdx = currentStatIdx - #dataNames
    end
    SwitchReceivers(I, currentStatIdx)
    lastSwitchTime = time
  end
  for i=0, I:GetNumberOfTargets(mainframeChannel) - 1 do
    local t = I:GetTargetInfo(mainframeChannel, i)
    if targets[t.Id] and targets[t.Id].missingData == 0 then
      I:LogToHud("Target "..i.." identified as "..targets[t.Id].identity.name)
    end
  end
end

function SwitchReceivers(I, idx)
  for name, rec in pairs(receivers) do
    if name == dataNames[idx] then
      I:Component_SetIntLogic(23, rec, mainframeChannel)
    else
      I:Component_SetIntLogic(23, rec, 0)
    end
  end
end

function GatherData(I, currentStat)
  for i=0, I:GetNumberOfTargets(mainframeIdx) - 1 do
    local t = I:GetTargetInfo(mainframeIdx, i)
    if not targets[t.Id] then
      targets[t.Id] = { missingData = #dataNames, stats = {} }
    end
    if targets[t.Id].missingData > 0 then
      if not targets[t.Id].stats[currentStat] then
        targets[t.Id].stats[currentStat] = t.Score
        I:Log(t.Score)
        I:Log(currentStat)
        targets[t.Id].missingData = targets[t.Id].missingData - 1
      end
    end
    if targets[t.Id].missingData == 0 then
      targets[t.Id].identity = FindClosest(targets[t.Id].stats, enemyData, sqrThreshold)
    end
  end
end

function FindClosest(s, dict, threshold)
  local best
  local bestE = 1/0
  for i, datum in ipairs(dict) do
    local stats = datum.stats
    local e = StatDistance(s, stats)
    if e < bestE then
      best = datum
      bestE = e
    end
  end
  if not threshold or bestE < threshold then
    return best
  end
  return nil
end

-- calculates L2 distance between data
function StatDistance(s0, s1)
  local sum = 0
  for i, name in ipairs(dataNames) do
    local d = s0[name] - s1[name]
    sum = sum + d * d
  end
  return sum
end

-- copy ShipData.lua here
